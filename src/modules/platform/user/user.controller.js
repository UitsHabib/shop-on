...
const Profile = require(path.join(
    process.cwd(),
    "src/modules/platform/profile/profile.model"
));
const Role = require(path.join(
    process.cwd(),
    "src/modules/platform/role/role.model"
));
const ProfilePermission = require(path.join(
    process.cwd(),
    "src/modules/platform/permission/profile-permission.model"
));
const RolePermission = require(path.join(
    process.cwd(),
    "src/modules/platform/permission/role-permission.model"
));
const Permission = require(path.join(
    process.cwd(),
    "src/modules/platform/permission/permission.model"
));
const PermissionService = require(path.join(
    process.cwd(),
    "src/modules/platform/permission/permission-service.model"
));
const Service = require(path.join(
    process.cwd(),
    "src/modules/platform/service/service.model"
));
...
async function updateSignedInUserProfile(req, res) {
    try {
        const { first_name, last_name, password, phone, profile_id, role_id } =
            req.body;
...
async function getUsers(req, res) {
    try {
        ...
        if (page < 0)
            return res.status(404).send("page must be greater or equal 1");
...
async function createUser(req, res) {
    try {
        const { first_name, last_name, email, password, profile_id, role_id } =
            req.body;
...
async function deleteUser(req, res) {
    try {
        ...
        const {
            password: Password,
            password_updated_at: PasswordUpdateAt,
            ...userInfo
        } = user.dataValues;
